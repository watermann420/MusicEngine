...
public class SimpleSynth : ISynth
{
    ...
    public int Read(float[] buffer, int offset, int count)
    {
        ...
        {
            // Calculate LFO value
            float lfoValue = WaveformGenerator.Basic(LfoWaveform, _lfoPhase, 0.5f);
            ...
            // De-zipper smoothing to reduce clicks on fast changes
            mixL = _lastOutL + _outputSmoothing * (mixL - _lastOutL);
            mixR = _lastOutR + _outputSmoothing * (mixR - _lastOutR);
            _lastOutL = mixL;
            _lastOutR = mixR;
...
#pragma warning restore CS0169
        ...
        public (float left, float right) Process(int sampleRate, float pitchMod, float filterMod, float pwMod, SimpleSynth synth)
        {
            ...
            // Process envelopes
            _ampEnv = EnvelopeGenerator.Process(_ampStage, _ampEnv, synth.Attack, synth.Decay, synth.Sustain, synth.Release, sampleRate, ref _ampStage);
            ...
            const float cutoffSmoothRate = 0.02f;
